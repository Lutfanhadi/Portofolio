...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const toggleDarkModeHp = document.getElementById("toggleDarkModeHp");
    const buttonCv = document.getElementsByClassName("buttonCv")
    ...
    toggleDarkModeHp.addEventListener("click", () => {
        if (containerbody.classList.contains("bg-dark")) {
          noneAktif();
        } else {
          aktif();
    ...
    function aktif () {
        ...
        toggleDarkModeHp.innerHTML = lightMode;
        ...
        for(let valueTextTitle = 0 ; valueTextTitle < textTitle.length ; valueTextTitle++){
            textTitle[valueTextTitle].style.color = "black";
        }

        for(let valueSection = 0 ; valueSection < containerSection.length ; valueSection++){
        ...
        for(let valueButtonCv = 0 ; valueButtonCv < buttonCv.length ; valueButtonCv++){
            buttonCv[valueButtonCv].classList.add("bg-white", "dark:hover:bg-gray-600");
            buttonCv[valueButtonCv].classList.remove("bg-gray-900", "dark:hover:bg-gray-500");
        }
        
         for (let valueHoverText = 0; valueHoverText < hoverLinksText.length; valueHoverText++) {
           hoverLinksText[valueHoverText].classList.add("dark:hover:text-white");
    ...
    function noneAktif () {
      ...
      toggleDarkModeHp.innerHTML = darkMode;
      containerSidebar.style.background = "#81888FC7";
      ...
      for(let valueButtonCv = 0 ; valueButtonCv < buttonCv.length ; valueButtonCv++){
            buttonCv[valueButtonCv].classList.add("bg-gray-900", "dark:hover:bg-gray-500");
        buttonCv[valueButtonCv].classList.remove("bg-white", "dark:hover:bg-gray-600");
        }

      for (let valueHoverText = 0; valueHoverText < hoverLinksText.length; valueHoverText++) {
        hoverLinksText[valueHoverText].classList.remove("dark:hover:text-white");
        hoverLinksText[valueHoverText].classList.add("dark:hover:text-black");
      }
